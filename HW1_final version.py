...
def call_theta(S,K,r,T,sigma):
    d1 = (np.log(S/K)+(r+0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    d2 = (np.log(S/K)+(r-0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    calltheta = -((S*si.norm.pdf(d1)*sigma)/(2*np.sqrt(T))) - r*K*np.exp(-r*T)*si.norm.cdf(d2)
    return(calltheta)
...
def put_theta(S,K,r,T,sigma):
    d1 = (np.log(S/K)+(r+0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    d2 = (np.log(S/K)+(r-0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    puttheta = -((S*si.norm.pdf(d1)*sigma)/(2*np.sqrt(T))) + r*K*np.exp(-r*T)*si.norm.cdf(-d2)
    return(puttheta)

#call rho
def call_rho(S,K,r,T,sigma):
    d2 = (np.log(S/K)+(r-0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    callrho = K*T*np.exp(-r*T)*si.norm.cdf(d2)
    return(callrho)

#put rho
def put_rho(S,K,r,T,sigma):
    d2 = (np.log(S/K)+(r-0.5*sigma**2)*T)/(sigma*np.sqrt(T))
    putrho = -K*T*np.exp(-r*T)*si.norm.cdf(-d2)
    return(putrho)
